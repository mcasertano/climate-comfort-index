...
def calculate_comfort_score(df, preferences):
    ...
    df['temperature_score'] += 1 - np.abs(df['Coldest month\'s avg low (F)'] - preferences['preferred_winter_low']) / 100
    ...
    humidity_map = {'low': 0.3, 'moderate': 0.5, 'high': 0.8}
    ...
    sunshine_map = {'less sun': 0.4, 'balanced': 0.7, 'more sun': 1}
    df['sunshine_score'] = 1 - np.abs(df['Annual Sunshine - Percentage of Possible'] / 100 - sunshine_map[preferences['sun_preference']])

    rainfall_map = {'low': 0.1, 'moderate': 0.3, 'high': 0.6}
    df['rainfall_score'] = 1 - np.abs(df['Annual rainfall (in)'] / 100 - rainfall_map[preferences['annual_rainfall_preference']])
    # rainfall_season_map = {'low': 0.05, 'moderate': 0.1, 'high': 0.25}
    # if preferences['rainfall_season_preference'] == 'summer':
    #     df['rainfall_score'] = 1 - np.abs(df['Summer rainfall (in)'] / 100 - rainfall_season_map[preferences['rainfall_season_preference']])
    # elif preferences['rainfall_season_preference'] == 'winter':
    #     df['rainfall_score'] = 1 - np.abs(df['Winter rainfall (in)'] / 100 - rainfall_season_map[preferences['rainfall_season_preference']])
